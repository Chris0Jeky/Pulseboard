...
- [ ] q-9: **Map a first repository for GitHub evidence?** Issue #20's first slice ships `observatory/src/github-map.mjs` with an empty mapping and no hosted token, so the Desk reads GitHub evidence as unmapped and nothing contacts GitHub. Do you want Alibi mapped first? If so, confirm its repository and the workflow(s), branch and deployment environment that count, and whether a fine-grained read-only token (Actions, Deployments, Contents metadata; this repository only) may be set as `GITHUB_EVIDENCE_TOKEN` locally or as a Worker secret. The agent then adds the verified numeric ids in a reviewed PR; setting a hosted secret stays your action.
