...
export function createObserver(config, runtime = globalThis) {
  ...
  function track(event, options = {}) {
    ...
    const now = Date.now();
    if (lastEvent && now - lastEvent > 1800000) {
      session = runtime.crypto.randomUUID();
      seq = 0;
    }
    lastEvent = now;
    const e = {
    ...
    if (
      !validateEvent(e, project) ||
      Object.keys(options).some(
        (key) => !['route', 'release', 'value'].includes(key),
      )
    ) {
      stats.dropped++;
      return false;
    }
    if (queue.length >= 100 || failures >= 3 || requests >= 120) {
      stats.dropped++;
      return false;
    }
    queue.push(e);
    ...
    while (queue.length && failures < 3 && requests < 120) {
      ...
      try {
        post(batch, { keepalive: true })?.then?.(
          (response) => {
            if (generation === epoch) {
              if (response?.ok) stats.sent += batch.length;
              else stats.dropped += batch.length;
            }
          },
          () => {
            stats.failures++;
            if (generation === epoch) stats.dropped += batch.length;
          },
        );
      } catch {
        stats.failures++;
