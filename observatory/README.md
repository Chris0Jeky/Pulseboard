...
`wrangler.jsonc` carries a 15-minute cron trigger, so **synthetic probing of every registered project origin begins as soon as a deploy includes it**, before any browser integration exists. `COLLECT_ENABLED` gates browser event admission only, together with `COLLECT_PROJECTS` (a comma-separated list of registered ids; a project not listed is refused with 503 even while the switch is on, so one pilot never opens every origin's budget to forged requests); neither gates probing. Review `src/projects.mjs` before deploying, or empty the `triggers.crons` list in `wrangler.jsonc` until you have. The hosted Worker has carried the cron since 2026-09-10; its first publication that day shipped with the list empty, and `docs/HOSTING.md` records whether Cloudflare has been observed invoking it.

Generate a unique, high-entropy read token of at least 32 characters. Keep it in a password manager. It grants portfolio-wide aggregate access; this version is for a single operator, not a multi-tenant service. Set it as a Worker secret, never a public build variable. Rotate by replacing the secret. Dashboard assets contain no private data; `/v1/summary` requires the token. `/healthz` is liveness only; `/readyz` checks the migrated database and validates every exact `COLLECT_PROJECTS` id against the public-project registry. Unknown, case-mismatched, semicolon-joined or local-only ids make readiness return 503 and fail browser admission closed.

Before collection: review `src/projects.mjs`, current privacy notices and the actual deployed origins. Replace or remove anything not owned by you. Keep Taskdeck's origin null. Test a preview deployment, inspect a received payload, and confirm refusal after revocation. Then change `COLLECT_ENABLED` to `"true"`, list the pilot's id in `COLLECT_PROJECTS` and redeploy. The defaults are `"false"` and an empty list.
...
The generated script has an empty endpoint until explicitly activated. Only then does it show a native **Usage sharing** control. Consent is per project and collector endpoint, expires after 90 days (re-checked before every send, so a tab or installed app left open past the deadline stops and shows sharing as off), and can be withdrawn at any time. There is no persistent analytics identifier. Reconfiguring the destination requires fresh consent.
...
Delivery is intentionally at-most-once from the SDK: each event is admitted at most once and sent at most twice. When the page is hidden, whatever is still queued is handed to the browser with `fetch(..., { keepalive: true })`, so a tracked click that navigates away is not simply discarded; its outcome is unobservable and it is never retried. A batch still in flight at that moment is reissued once, with the same event IDs, the same way; the collector ignores the duplicate IDs (they still spend admission budget). Keepalive bodies stay within the browser's 64 KiB in-flight cap, in-flight batch first; what does not fit stays queued, sent later after a bfcache restore and otherwise discarded with the page. `navigator.sendBeacon` is deliberately not used, because it attaches cookies and cannot omit credentials. A failed batch is dropped, never replayed after later consent. A request that times out (10 s for the first request of a page, 5 s after) is counted as `unknown`, not failed: the collector may have admitted it, so it is neither retried nor counted toward the circuit breaker, which opens after three genuine failures. A successful fetch acknowledgment can still be lost. SDK `status()` exposes its local sent/dropped/unknown event counters and failed-request counter; these are not a population-wide loss estimate.
