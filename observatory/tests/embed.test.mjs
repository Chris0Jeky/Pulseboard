...
function setup({
  ...
  const runtime = Object.assign(element('window'), {
    ...
    AbortController,
    location: {
      origin: projects.mdviewer.origin,
      protocol: 'https:',
      pathname: '/',
    },
    localStorage: {
  ...
  if (contextProvider !== undefined) runtime.OBSERVATORY_CONTEXT = contextProvider;
...
const grant = async (harness) => {
  harness.checkbox.checked = true;
  harness.checkbox.emit('change');
  await settle();
};
const sent = (calls) =>
  calls.flatMap((call) => JSON.parse(call[1].body).events);
const sentEvents = (calls) => sent(calls).map((event) => event.event);
...
test('granting persists the choice under the project and endpoint key and reports one page view', async () => {
  ...
  assert.ok(
    stored.until > Date.now() + 89 * DAY &&
      stored.until <= Date.now() + 90 * DAY,
  );
...
test('host context supplies a bounded route and release for each event', async () => {
  let context = { route: 'editor', release: '0.11.3' };
  const harness = setup({
  ...
  await grant(harness);
  assert.deepEqual(
    sent(harness.calls).map(({ route, release }) => [route, release]),
    [['editor', '0.11.3']],
  );
  context = { route: 'private', release: 'private-build' };
...
test('a throwing context provider fails closed to registered defaults', async () => {
  const harness = setup({
    contextProvider: () => {
  ...
  await grant(harness);
  const [event] = sent(harness.calls);
  assert.equal(event.route, 'home');
  assert.equal(event.release, 'unattributed');
  harness.facade.dispose();
});
test('an expired choice grants nothing and an inflated expiry is capped at 90 days', async () => {
  const expired = setup({
    storage: new Map([
      [
        'pulseboard:consent:v1:mdviewer:https://collector.example/v1/collect/mdviewer',
        JSON.stringify({ allow: true, until: Date.now() - 1000 }),
      ],
    ]),
  });
  assert.equal(expired.checkbox.checked, false);
  ...
  const inflated = setup({
    storage: new Map([
      [
        'pulseboard:consent:v1:mdviewer:https://collector.example/v1/collect/mdviewer',
        JSON.stringify({ allow: true, until: Date.now() + 3650 * DAY }),
      ],
    ]),
  });
  assert.equal(inflated.checkbox.checked, true);
  assert.ok(
    JSON.parse(inflated.storage.get(inflated.key)).until <=
      Date.now() + 90 * DAY,
  );
...
test('a reconfigured collector endpoint requires fresh consent', async () => {
  const storage = new Map(),
    first = setup({ storage });
  await grant(first);
  first.facade.dispose();
  const moved = setup({
    storage,
    endpoint: 'https://elsewhere.example/v1/collect/mdviewer',
  });
  assert.equal(moved.checkbox.checked, false);
...
test('an invalid click selector cannot throw inside a listener on the host document', async () => {
  const harness = setup({
    clicks: [
      { selector: ':::not-a-selector', event: 'export.print_requested' },
    ],
  });
  await grant(harness);
  assert.doesNotThrow(() =>
    harness.document.emit('click', {
      target: {
        closest: (selector) => {
          throw new SyntaxError(selector);
        },
      },
    }),
  );
...
test('a tracked click that navigates away is handed over on pagehide', async () => {
  ...
  assert.deepEqual(sentEvents(harness.calls), [
    'page.view',
    'export.print_requested',
  ]);
