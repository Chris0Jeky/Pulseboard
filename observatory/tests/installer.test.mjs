...
test('the artifact publishes the client contract only, never operator data', () => {
  ...
  for (const operatorOnly of [
    ...
    assert.equal(
      serialised.includes(operatorOnly),
      false,
      operatorOnly + ' leaked into the artifact',
    );
  const config = JSON.parse(serialised);
  assert.deepEqual(Object.keys(config.project).sort(), [
    'events',
    'measurements',
    'releases',
    'routes',
...
test('Alibi artifact publishes only its bounded context handle and registered release', () => {
  ...
  assert.equal(serialised.includes('ALIBI_CONFIG.version'), false);
});
test('embed options are an allowlist, not an arbitrary override', () => {
  assert.throws(
    () => buildEmbed('mdviewer', { origin: 'https://attacker.test' }),
    /Unsupported embed option: origin/,
  );
  assert.throws(
    () => buildEmbed('mdviewer', { project: { events: ['anything'] } }),
    /Unsupported embed option: project/,
  );
  assert.throws(
    () => buildEmbed('mdviewer', { scopePath: '/' }),
    /Unsupported embed option: scopePath/,
  );
  assert.equal(
    buildEmbed('mdviewer', { route: 'editor', clicks: [] }).includes(
      '"route":"editor"',
    ),
    true,
  );
...
test('installer is repeatable, refuses edited files, and confines paths', () => {
  ...
  try {
    const a = install('mdviewer', root, 'public/observer.js');
    const b = install('mdviewer', root, 'public/observer.js');
    assert.deepEqual(a, b);
    writeFileSync(path.join(root, 'public/observer.js'), 'my work');
    assert.throws(
      () => install('mdviewer', root, 'public/observer.js'),
      /refusing overwrite/,
    );
    assert.throws(
      () => install('mdviewer', root, '../escape.js'),
      /relative path/,
    );
    assert.equal(
      readFileSync(path.join(root, 'public/observer.js'), 'utf8'),
      'my work',
    );
  } finally {
...
test('installer refuses symlink escape', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'observatory-test-')),
    outside = mkdtempSync(path.join(tmpdir(), 'observatory-outside-'));
  try {
    symlinkSync(outside, path.join(root, 'linked'));
    assert.throws(
      () => install('mdviewer', root, 'linked/embed.js'),
      /Symlink/,
    );
  } finally {
    rmSync(root, { recursive: true });
...
test('two installs in one repository keep separate lock entries', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'observatory-test-'));
  try {
    const first = install('mdviewer', root, 'public/observer.js'),
      second = install('commitatlas', root, 'site/observer.js');
    const lock = JSON.parse(
      readFileSync(path.join(root, 'observatory.lock.json'), 'utf8'),
    );
    assert.deepEqual(Object.keys(lock.installs).sort(), [
      'public/observer.js',
      'site/observer.js',
    ]);
...
test('a lock file in the original single-entry shape still owns its target', () => {
  ...
  try {
    ...
    writeFileSync(
      path.join(root, 'observatory.lock.json'),
      JSON.stringify({
        version: '0.1.0',
        project: 'mdviewer',
        target: 'public/observer.js',
        sha256,
      }),
    );
    assert.equal(
      install('mdviewer', root, 'public/observer.js').sha256,
      sha256,
    );
    assert.deepEqual(
      Object.keys(
...
test('a malformed artifact is rejected before any file is written', () => {
  ...
  assert.throws(
    () => assertArtifactShape('export function mount() {}\n'),
    /module statement/,
  );
  assert.throws(
    () => assertArtifactShape('const limit = MAX_BYTES;\n'),
    /server-only constants/,
  );
});
test('Taskdeck cannot accidentally acquire a public embed', () =>
  assert.throws(
    () => buildEmbed('taskdeck'),
    /no public collection origin/,
  ));
