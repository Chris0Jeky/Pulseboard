...
test('aggregate reader excludes legacy rows and other days', async t => {
  ...
  assert.equal(result.schema, 'pulseboard.statistics/3');
  ...
  for (const name of ['country', 'device', 'source', 'visit']) assert.deepEqual(result.dimensions[name], [{ value: 'unknown', n: 5 }]);
