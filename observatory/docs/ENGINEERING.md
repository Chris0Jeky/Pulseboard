...
Queries are bounded to a 1-, 7- or 14-day window and indexed for time and session flow. A connected desk polls `/v1/portfolio` every 30 seconds while its tab is visible (`REFRESH_MS` in `public/dashboard.mjs`, the only place the interval is named), so one open tab makes up to 120 automatic portfolio reads per hour (plus the same number of `/v1/statistics/alibi` reads while the Usage view is the open view) plus one read per manual refresh and per return to a visible tab (those are guarded against overlap, not against frequency), and each read runs a fixed set of window-wide aggregate scans grouped across every project. The poll is not a background drip: `refresh` returns early when `document.hidden`, the `visibilitychange` handler cancels the in-flight read and the pending timer, and the timer is only re-armed while a token is held and the tab is visible — so a forgotten hidden tab costs nothing until it is looked at again. Multiply by the number of open desks before treating that ceiling as small. Check D1's reported rows_read / rows_written in the actual account before changing `REFRESH_MS`, caps or window size. Prefer aggregate tables and explicit retention policy before increasing volumes substantially.

GitHub evidence is not polled. One explicit dossier read makes at most 12 GitHub requests (`LIMITS` in `src/github.mjs`), follows at most 2 pages per list, reads at most 256 KiB per response and waits at most 5 seconds per request. A project is re-read from GitHub at most once every 10 minutes; within that floor the connector answers from its in-memory ETag cache (at most 128 projected entries, insertion ordered, keys are request paths), and a 304 still spends budget. It stops at a rate-limit response until the reset time (capped at one hour) and pauses early when fewer than 50 requests remain in the hourly quota. Nothing is written to D1.
...
Amended again 2026-09-26 (plan version 2, HUMAN_TODO q-20): diagnostics and journeys are default-on outside the EEA and need an OK inside it, product events carry bounded open JSON, detailed data is kept 90 days and aggregates 400 days. `USAGE_PLAN.md`, "Amendments to ENGINEERING.md", lists every statement this replaces.
