...
export function assertArtifactShape(content) {
  ...
  if (leaked)
    throw new Error(
      'Generated artifact still contains a module statement: ' +
        leaked.slice(0, 60),
    );
  if (/MAX_BYTES|MAX_BATCH/.test(content))
    throw new Error(
      'Generated artifact still contains server-only constants',
    );
  return content;
}
...
export function buildEmbed(id, options = {}) {
  if (!Object.hasOwn(projects, id))
    throw new Error('Register the project in src/projects.mjs first');
  const unsupported = Object.keys(options).find(
    (key) => !EMBED_OPTIONS.includes(key),
  );
  if (unsupported)
    throw new Error('Unsupported embed option: ' + unsupported);
  const project = projects[id];
  if (!project.origin)
    throw new Error('This project deliberately has no public collection origin');
  ...
  if (project.contextGlobal !== undefined) {
    if (!/^[A-Z][A-Z0-9_]{0,63}$/.test(project.contextGlobal))
      throw new Error('Project context global must be a bounded identifier');
  ...
  if (config.endpoint) {
    ...
    if (
      url.protocol !== 'https:' ||
      url.pathname !== '/v1/collect/' + id ||
      url.search ||
      url.hash ||
      url.username ||
      url.password
    )
      throw new Error('Expected the exact HTTPS project collector endpoint');
  }
  ...
  const contract = read('../src/contracts.mjs')
    .split('export function validateBatch')[0]
    .replace(/^export const MAX_(?:BYTES|BATCH) =.*\n/gm, '')
    .replace(/^export /gm, '');
  const browser = read('../src/browser.mjs')
    .replace(/^import .*;\n/gm, '')
    .replace(/^export /gm, '');
...
function readLock(lockPath, present) {
  ...
  if (
    raw?.installs &&
    Object.getPrototypeOf(raw.installs) === Object.prototype
  )
    return { ...empty, installs: { ...raw.installs } };
  if (typeof raw?.target === 'string')
    return {
      ...empty,
      installs: {
        [raw.target.split(/[\\/]/).join('/')]: {
          project: raw.project,
          sha256: raw.sha256,
        },
      },
...
export function install(id, root, target, endpoint = '') {
  const base = realpathSync(root),
    full = path.resolve(base, target);
  if (
    path.isAbsolute(target) ||
    !full.startsWith(base + path.sep) ||
    target.split(/[\\/]/).includes('..')
  )
    throw new Error('Target must be a relative path inside the repository');
  ...
  try {
    ...
    throw new Error('Symlink leaves repository');
  }
  ...
  if (full === lockPath || lockStat?.isSymbolicLink())
    throw new Error('Refusing reserved or symlinked lock path');
  const lock = readLock(lockPath, !!lockStat),
    key = target.split(/[\\/]/).join('/'),
    owned = lock.installs[key];
  if (
    targetStat &&
    (!targetStat.isFile() ||
      !owned ||
      owned.sha256 !== digest(readFileSync(full)))
  )
    throw new Error(
      'Existing file has local edits or is unowned; refusing overwrite',
    );
  ...
  writeFileSync(
    lockPath,
    JSON.stringify(
      { version: '0.1.0', source: SOURCE, installs: lock.installs },
      null,
      2,
    ) + '\n',
  );
...
if (
  ...
  if (!id || !root || !target) {
    console.error(
      'Usage: node adapters/build-embed.mjs PROJECT REPOSITORY_ROOT RELATIVE_OUTPUT [HTTPS_ENDPOINT]',
    );
    process.exitCode = 2;
  } else
    console.log(
      JSON.stringify(install(id, root, target, endpoint), null, 2),
    );
}
