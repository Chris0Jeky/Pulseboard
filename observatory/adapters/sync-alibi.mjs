...
function snapshotManagedFiles(root, relatives) {
...
export function renderAlibiReleaseRegistry(releases) {
  ...
  return '// Generated by npm run sync:alibi -- --write <alibi-repository>.\n'
...
export function syncAlibi(rootInput, { mode = 'check' } = {}) {
  ...
  if (mode === 'check') {
    if (!wasRegistered) throw new Error(`Alibi ${alibi.version} is missing from the Pulseboard collector contract; run npm run sync:alibi -- --write <alibi-repository>`);
    const generated = buildExpectedEmbed(current);
    if (normalise(alibi.artifact) !== normalise(generated)) throw new Error('Alibi Observatory artifact is stale; regenerate it with npm run sync:alibi -- --write <alibi-repository>');
    return { mode, project: 'alibi', packageVersion: alibi.version, registered: true, releases: current, target: TARGET, sha256: sha256(generated), bytes: Buffer.byteLength(generated) };
  }
  ...
  const snapshots = [
    ...
    { full: registryFile, existed: true, bytes: Buffer.from(registryBytes, 'utf8') },
  ];
  try {
    ...
    return { mode, project: 'alibi', packageVersion: alibi.version, registered: true, addedRelease: wasRegistered ? null : alibi.version,
      releases: next, target: TARGET, sha256: installed.sha256, bytes: Buffer.byteLength(generated) };
  } catch (error) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(fileURLToPath(import.meta.url))) {
  ...
  if (!['--check', '--write'].includes(mode) || !root) {
    console.error('Usage: node adapters/sync-alibi.mjs --check|--write <alibi-repository>');
    process.exitCode = 2;
  } else {
    try {
      const result = syncAlibi(root, { mode: mode.slice(2) });
