...
export function mountObserver(config, create, runtime = globalThis) {
  ...
  if (
    config.publicFlag &&
    runtime[config.publicFlag.global]?.[config.publicFlag.key] !==
      config.publicFlag.expected
  )
    return null;
  const observer = create(config, runtime);
  const key =
      'pulseboard:consent:v1:' + config.id + ':' + config.endpoint,
    CONSENT_MS = 90 * 86400000;
  let granted = false,
    overdue = false;
  // A stored expiry is never trusted past 90 days from now; a tampered or corrupt one cannot grant indefinite consent.
  try {
    const stored = JSON.parse(runtime.localStorage.getItem(key) || 'null'),
      cap = Date.now() + CONSENT_MS;
    const until = Number.isFinite(stored?.until)
      ? Math.min(stored.until, cap)
      : 0;
    granted = stored?.allow === true && until > Date.now();
    overdue = granted && stored.until > cap;
  } catch {
    /* Session choice still works without storage. */
  }
  function hostContext() {
    if (!config.contextGlobal) return {};
    try {
      const provider = runtime[config.contextGlobal];
      const value = typeof provider === 'function' ? provider() : provider;
      if (!value || Object.getPrototypeOf(value) !== Object.prototype) return {};
      return {
        ...(config.project.routes.includes(value.route)
          ? { route: value.route }
          : {}),
        ...(config.project.releases.includes(value.release)
  ...
  note.textContent =
    'Optional: share a small set of action counts with ' +
    new URL(config.endpoint).hostname +
    '. No document text, filenames, form values or browsing history is sent. Raw events expire after 14 days. Your choice lasts 90 days on this browser.';
  const label = document.createElement('label'),
    checkbox = document.createElement('input');
  checkbox.type = 'checkbox';
  label.append(
    checkbox,
    document.createTextNode(' Share basic usage for this site'),
  );
    ...
    if (persist) {
      try {
        runtime.localStorage.setItem(
          key,
          JSON.stringify({
            allow: checkbox.checked,
            until: Date.now() + CONSENT_MS,
          }),
        );
      } catch {
        status.textContent += ' This choice could not be saved.';
      }
    }
    // One page view per page, on the first time sharing is on: re-ticking the box is not another visit.
    if (checkbox.checked && !announced) {
      announced = true;
      track('page.view');
      void observer.flush();
    }
  ...
  details.append(title, note, label, status);
  document.body.append(details);
  apply(granted, overdue);
  ...
  const click = (event) => {
    try {
      ...
      /* A bad selector must never throw inside a listener on the host document. */
    }
  };
  runtime.addEventListener('error', error);
  runtime.addEventListener('unhandledrejection', error);
  ...
  return {
    ...
    flushOnHide: observer.flushOnHide,
